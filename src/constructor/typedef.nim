...
macro typeDef*(name: untyped, body: untyped): untyped =
  ...
  let
    ...
    exportedName = if exported: postfix(name,
        "*") else: name #if we want to export the type make a postfix
    ...
    if node.kind in {nnkIdent, nnkCommand, nnkPar}:
      ...
      for varName in iterate:
        ...
        if not isProp:
          ...
          let
            setterName = if setExported: postfix(ident($varName & "="),
                "*") else: ident($varName & "=")
            getterName = if getExported: postfix(varName, "*") else: varName
          let
            valueIdent = ident("value")
            setter = quote do:
              proc `setterName`(`lowerName`: var `name`,
                  `valueIdent`: `typeVal`) =
